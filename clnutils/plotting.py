...
def id_generator(df, hole_col, from_col, to_col, composite = False):
    ...
    if composite:
        ...
        return df[hole_col] + " " + df[from_col].astype(str), df[hole_col] + " " + df[to_col].astype(str) 
...
def lith_linear_proportion(df, lith_col,
    ...
    '''
    if percent:
        temp_series = (df.groupby(by=lith_col)[interval_col].sum()/df[interval_col].sum())*norm_to
    else:
...
def pyramid_plotter(df, env_col, exp_col, raw_data=False):
    ...
    _=sns.barplot(x=df[env_col],
            y=df.index,
            ax=ax1,
            color="r",
            label="Environmental",
            lw=0)
    _=sns.barplot(x=df[exp_col],
            y=df.index,
            ax=ax1,
            color="b",
            label="Exploration",
    ...
    for bar in ax1.patches:
        ...
        if height < 0:
            if np.abs(height) > (limx_r/2):
                ax1.text(x=height,y=bar.get_y() + bar.get_height() / 2, s=lab, fontsize=17, color="w",
                        ha="left", va="center")
            else:
                ax1.text(x=height,y=bar.get_y() + bar.get_height() / 2, s=lab, fontsize=17, color="k",
                        ha="right", va="center")
        else:
            if np.abs(height) > (limx_r/2):
                ax1.text(x=height,y=bar.get_y() + bar.get_height() / 2, s=lab, fontsize=17, color="w",
...
def bplot_lith_prop(df,lithorder, xlab, title,proprt,group_by="Lithology (Relog)",
                    lines=False, lineloc=(.2,2,3), pal="turbo",
                    scale_log = False):
    ...
    if scale_log:
        ...
        lab_x = a-(x_range/15)*.2 # lab_x for plotting anotations
    axx.set_xlabel(xlab, fontsize=25)
    axx.set_ylabel(None)
    y_max,y_min = axx.get_ylim()
    axx.set_ylim(y_max+.5, y_min-.5)
    axx.tick_params(axis='both', which='major', labelsize=25)
    if lines:
        axx.vlines(lineloc, ymin=0, ymax=1,
                   colors="#980043", linestyles="--",
