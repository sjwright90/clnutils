...
def overlap(df, hole_col="Drill Hole", t1="From", t2="To", samp_id1="Sample ID", intv = "Interval"):
...
def id_ovlp_to_drop(df,
    ...
    temp["sample_to_drop"] = np.where(temp[pct_up]>temp[pct_lwr], temp[samp_up], temp[samp_lwr])
...
def drop_metals_aba_dup(df, which="Metals", ignore_na=True, custom_range = None):
    ...
    if ignore_na:
        return df[(~df.duplicated(subset=cols))|df[cols].isna().all(1)].reset_index(drop=True)
    else:
...
def find_overlap(minval, maxval, targetfrom, targetto):
    ...
    return np.where(targetfrom<maxval, True,False), np.where(targetto>minval,True,False)

def test_continuity(discontinuity,
                    env_holes,
                    expholeid="HOLEID",
                    expfrm="SAMPFROM",
                    expto="SAMPTO",
    ...
    for hole in discontinuity[expholeid].unique():
        ...
        exp_min,exp_max = get_bounds(\
            tempexp[[expfrm,expto]])
        env_min,env_max = get_bounds(\
            tempenv[[envfrm,envto]])
        # determine if any overlap b/t the dfs present
        if no_overlapping(exp_min,
                          exp_max,
                          env_min,
                          env_max):
        ...
        tempenv["under"],tempenv["over"] = find_overlap(exp_min,exp_max,tempenv[envfrm],tempenv[envto])
        tempenv["either"] = np.where((tempenv.under==1)|(tempenv.over==1),True,False)
        ...
        for idx,row in tempenv.iterrows():
            ...
            for idx_x, row_x in tempexp.iterrows():
                ...
                if not no_overlapping(row[envfrm],row[envto],row_x[expto],row_x[expfrm]):
